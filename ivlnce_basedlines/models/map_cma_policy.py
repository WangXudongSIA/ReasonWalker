...
class MapCMAPolicy(ILPolicy):
    ...
    def act_iterative(
        ...
        if observations['depth'] is not None:
            action_is_1 = action == 1
            if action_is_1.any():
                depth_mean = torch.mean(observations['depth'],
                                        dim=(1, 2, 3))
                close_to_zero = depth_mean < 0.01
                if close_to_zero.any():
                    ...
                    for i in range(batch_size):
                        if action_is_1[i] and close_to_zero[i]:
                            if left_mean[i] > right_mean[i]:
                                action[i] = 2
                            elif right_mean[i] > left_mean[i]:
                                action[i] = 3
                            print("Preventing lockups!")
...
class MapCMANet(Net):
    ...
    def forward(
        ...
        print("rnn_states", rnn_states[:, 0:s1_layers].shape)
